...
export const removeDollars = (amounts: string[]): number[] => {
    ...
    return removed.map((num: string): number => (isNaN(+num) ? 0 : +num));
};
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    const messages2 = messages.filter(
        (message: string): boolean => !message.endsWith("?")
    );
    return messages2.map((message: string): string =>
    ...
    if (colors.length === 0) {
        ...
        return colors.every(
            (color: string): boolean =>
                color.toLowerCase().localeCompare("red") === 0 ||
                color.toLowerCase().localeCompare("blue") === 0 ||
                color.toLowerCase().localeCompare("green") === 0
        );
    }
}
